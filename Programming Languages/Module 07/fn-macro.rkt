...
; We're not using real macros but using regular functions to take expressions
; and return another expression, just like a regular macro would.
...
; Takes something like (eval-exp (product (list (const 2) (const 3))))
(define (product es)
  (if (null? es)
      (const 1)
      (multiply (car es) (product (cdr es)))))