...
PROCESS(task3_2, "Simulate trickle (Rime) in Cooja (1)");
AUTOSTART_PROCESSES(&task3_2);

PROCESS_THREAD(task3_2, ev, data) {
  PROCESS_EXITHANDLER(trickle_close(&trickle);)
  PROCESS_BEGIN();

  trickle_open(&trickle, 2*CLOCK_SECOND, 145, &callbacks);
